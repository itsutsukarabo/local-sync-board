...
**対象ファイル:**
- `app/app/(auth)/welcome.tsx`または設定画面
- `app/contexts/AuthContext.tsx`
- `app/lib/supabase.ts`

**作業内容:**
1. ホーム画面またはプロフィール設定にID再作成ボタンを追加
2. 現在のセッションをサインアウト
3. 新しい匿名アカウントを作成（`signInAnonymously()`）
4. 表示名の再設定を促す（welcomeスクリーンへ遷移）
5. 旧アカウントのデータ（着席中の部屋等）は引き継がない

**注意事項:**
- 実行中のゲーム（着席中）にこの機能を使うと、旧IDの着席が残るため、先に離席を強制する
- 確認モーダルで「すべてのゲームデータが失われます」と警告する
